...
extension HomeSceneViewAdaptor: UITableViewDataSource ,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            if let cell  = tableView.dequeueReusableCell(withIdentifier: String.sliderTableCellIdentifier(), for: indexPath) as? SliderTableViewCell{
            ...
            if let item = self.getItemInMaterialArray(at: indexPath.row){
                if item.type == String.news() {
                    if let cell  = tableView.dequeueReusableCell(withIdentifier: String.newsTableCellIdentifier(), for: indexPath) as? HomeTableViewCell{
                        cell.configureCell(item: item)
                        return cell
                    }
                    fatalError()
                } else if item.type == String.videos() {
                    if let cell  = tableView.dequeueReusableCell(withIdentifier: String.videoTableCellIdentifier(), for: indexPath) as? HomeVideoTableViewCell{
                    ...
                    if let cell  = tableView.dequeueReusableCell(withIdentifier: String.imageTableCellIdentifier(), for: indexPath) as? ImagesTableViewCell{
                    ...
                    if let cell  = tableView.dequeueReusableCell(withIdentifier: String.articleTableCellIdentifier(), for: indexPath) as? ArticleTableViewCell{
